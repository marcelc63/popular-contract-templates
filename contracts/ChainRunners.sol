...
/**
 * Solidity Contract Template based on @chain_runners
 * Source: https://etherscan.io/address/0x97597002980134bea46250aa0510c9b90d87a587
 *
...
contract ChainRunners is ERC721Enumerable, Ownable, ReentrancyGuard {
  ...
  // TODO: Update the supply, price, and early access settings here.
  ...
  function mint(uint256 tokenId) internal {
    // NOTE: Here is how random runners dna are generated. This determines the traits a runner will have.
    ChainRunnersTypes.ChainRunner memory runner;
    runner.dna = uint256(
      keccak256(
        abi.encodePacked(tokenId, msg.sender, block.difficulty, block.timestamp)
      )
    );
  ...
  function configureRunnerZero(uint256 _runnerZeroHash, uint256 _runnerZeroDNA)
    external
    onlyOwner
  {
    ...
    returns (string memory)
  {
    ...
    // NOTE: The renderer for the runner lives in a separate contract.
    // This means, the renderer can be upgraded.
    IChainRunnersRenderer renderer = IChainRunnersRenderer(
      renderingContractAddress
    );
