...
contract CorruptionsMetadata is Ownable, ICorruptionsMetadata {
  ...
  function draw(uint256 tokenId, uint256 amount)
    public
    pure
    returns (string memory)
  {
    ...
    parts[
      0
    ] = '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 205 335"><style>.base { fill: ';
    parts[1] = randomStrings[
      11 + (uint256(keccak256(abi.encodePacked("BGCOLOR", tokenId))) % 6)
    ];
    parts[
      2
    ] = '; font-family: monospace; font-size: 10px; }</style><rect width="100%" height="100%" fill="';
    parts[3] = randomStrings[
      27 + (uint256(keccak256(abi.encodePacked("FGCOLOR", tokenId))) % 6)
    ];
    parts[4] = '" />';
    parts[5] = output;
    parts[6] = "</svg>";
      ...
      bytes(
        string(
          abi.encodePacked(
            ...
            draw(tokenId, amount),
            '", "attributes": [{"trait_type": "Insight", "value": "',
            toString(amount),
            '"}]}'
          )
        )
